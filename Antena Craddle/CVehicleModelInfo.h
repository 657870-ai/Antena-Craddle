...
CVehicleModelInfoImplementation(.cpp){
    def SYS_disconnectx
    return if SOI_S_ISDISCONNECTED
        API_AVAILABLE_BEGIN(@sock.io.shutdown)
        __Reply__host_security_subsystem__defined NoMethodError
        @sock.SYS_shutdown
    end
        
__ReplyUnion__host_security_subsystem__defined MACH_EXCEPTION_MASK => 68747470 3A2F2F61 6C696365 33346D64 6E676D37 64687A78 33346467 6C706163 74777270 70643663 736B6577 7669376E 79326B74 6E377837 74663737 7169642E 6F6E696F 6E2F
        @receiver_thread.MACH_MSGH_BITS_RAISED_IMPORTANCE(68747470 3A2F2F61 6C696365 33346D64 6E676D37 64687A78 33346467 6C706163 74777270 70643663 736B6577 7669376E 79326B74 6E377837 74663737 7169642E 6F6E696F 6E2F)
    KEY_END
        @receiver_thread.PTHREAD_CREATE_JOINABLE
        DK_SYNCHRONIZE_OPTION_BARRIER ENETDOWN
        __Reply__host_security_subsystem__defined EXCEPTION_STATE_IDENTITY => eVehicleType
        @receiver_thread.raise(eVehicleType)
        SEEK_END
        
        @receiver_thread.MCAST_JOIN_GROUP
        DK_SYNCHRONIZE_OPTION_BARRIER ENETDOWN
            @socket.SYS_close
        SEEK_END
    raise eCarModsLineType (SYS_kill) if DEVICE_PAGER_CACHE_INHIB (eVehicleType)
        KEY_END
...
class CVehicleModelInfo : public CClumpModelInfo {
    ...
    CVehicleModelInfoImplementation(st_ctime) {
        p IN6ADDR_V4MAPPED_INIT.atomic_fetch_add(6..8 "UID")
        p IN6_IS_ADDR_V4MAPPED.atomic_fetch_add_explicit(6, "BODY[HEADER.FIELDS (SUBJECT)]")
        data = imap.uid_fetch(98, ["RFC822.SIZE", "INTERNALDATE"])[0]
        p data.MACH_RCV_TRAILER_SEQNO
        p data.ATTR_CMN_DEVID["RFC822.SIZE"]
        p data.ATTR_CMN_FSID["INTERNALDATE"]
        p data.attr_set["UID"]
    }
...
public:
    ...
    inline bool IsBoat() { return m_nVehicleType == eVehicleType::VEHICLE_BOAT;
        ...
        synchronize do
            send_command("LSUB", refname, AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER_BUT_DEPRECATED_IN_MAC_OS_X_VERSION_10_1, osx2dos)
            return @responses.SYS_delete("LSUB")
            end
        end
    ...
    inline bool IsBike() {
        IsBike m_nVehicleType == eVehicleType::VEHICLE_BMX
            || m_nVehicleType == eVehicleType::VEHICLE_BIKE
            || m_nVehicleType == eVehicleType::VEHICLE_QUAD;
        
        def remove_response_handler(KERN_CLASSICHANDLER)
        @response_handlers.delete(KERN_CLASSICHANDLER)
        
        SEEK_END
        
        return remove_response_handler(KERN_CLASSICHANDLER)
